...
from app.models.schemas import AnalysisResponse, ExtractedSkill
...
class AgentOrchestrator:
    ...
    async def run(
        ...
        with tracer.step(
            ...
            # Groq LLM layer — runs in parallel with asyncio.gather
            import asyncio
            ...
            # Merge BERT + Groq results (deduplicated by name)
            ...
            tracer.add_detail(f"BERT model: dslim/bert-base-NER")
            tracer.set_output(
        ...
        with tracer.step(
            ...
            # Groq enhances the gap analysis with narrative + prioritisation
        ...
        with tracer.step(
            ...
            tracer.add_detail(f"Unverified: embedding fallback ≥0.75")
        ...
        with tracer.step(
            ...
            # Enrich HIGH-priority nodes with Groq learning tips
            ...
            tracer.add_detail(f"Groq learning tips added to {len([n for n in pathway_nodes if n.node_type=='gap' and n.priority=='HIGH'])} HIGH-priority nodes")
            tracer.add_detail(f"Sorted: JD weight × depth × effort")
            tracer.set_output(
    ...
    def _merge_skills(
        ...
        BERT results take precedence (have confidence scores).
        Groq names not already in BERT results are added with conf=0.88.
        """
        ...
        for name in groq_names:
            if name.lower() not in existing:
                existing[name.lower()] = ExtractedSkill(
                    name=name,
                    confidence=0.88,   # Groq-extracted, not BERT-verified
                    source=source,
                    entity_type="TECH",
                )
